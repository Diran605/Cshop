                ...
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-gray-900">
                            {{ $editingId ? __('Edit Bulk Unit') : __('New Bulk Unit') }}
                        </h3>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr wire:key="bulk-unit-{{ $unit->id }}">
                                        ...
                                        <td class="px-4 py-3 text-sm text-right">
                                            <div class="inline-flex items-center gap-3">
                                                <button type="button" wire:click.stop.prevent="edit({{ $unit->id }})" class="text-indigo-600 hover:text-indigo-900">{{ __('Edit') }}</button>
                                                <button type="button" wire:click.stop.prevent="delete({{ $unit->id }})" onclick="return confirm('Are you sure you want to delete this record?')" class="text-red-600 hover:text-red-900">{{ __('Delete') }}</button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                @if ($bulkUnits->isEmpty())
                                    <tr>
                                        <td colspan="2" class="px-4 py-8 text-center text-sm text-gray-500">{{ __('No bulk units found.') }}</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
