<div class="ui-page">
            ...
            <div class="ui-card">
                <div class="ui-card-body">
                    ...
                    <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="ui-label">{{ __('Minimum Stock Alert Level') }}</label>
                            <input type="number" min="0" wire:model.defer="minimum_stock" class="mt-1 ui-input" placeholder="e.g. 10" />
                            @error('minimum_stock') <div class="mt-1 text-sm text-red-600">{{ $message }}</div> @enderror
                        </div>
                                    ...
                                    <tr wire:key="product-{{ $product->id }}">
                                        ...
                                        <td class="text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                @if ($mode === 'expired')
                                                <button type="button" wire:click="openBatchDetailsModal({{ $product->id }})" class="ui-btn-link text-xs">{{ __('View Batches') }}</button>
                                                @else
                                                @can('products.view')
            ...
            <div class="p-4 overflow-y-auto flex-1 min-h-0">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea wire:model.defer="edit_reason" rows="2" class="mt-1 ui-input" placeholder="{{ __('Why are you editing this product? (required if changing stock)') }}"></textarea>
    ...
    @if ($show_batch_details_modal)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-96 overflow-y-auto">
            <div class="flex items-center justify-between border-b border-slate-200 p-4 sticky top-0 bg-white">
                <h3 class="text-lg font-semibold text-slate-900">Expired Batches</h3>
                <button type="button" wire:click="closeBatchDetailsModal()" class="text-slate-500 hover:text-slate-700">
            ...
            <div class="p-4">
                @if (count($batch_details_batches) > 0)
                <div class="space-y-2">
                    @foreach ($batch_details_batches as $batch)
                    <div class="flex items-center justify-between p-3 border border-slate-200 rounded-md hover:bg-slate-50">
                        <div class="flex-1">
                            <div class="font-medium text-slate-900">
                                Batch: {{ $batch['batch_ref'] ?? 'N/A' }}
                            </div>
                            <div class="text-sm text-slate-600 mt-1">
                                <span>Expiry: <span class="font-semibold text-red-600">{{ $batch['expiry_date'] }}</span></span>
                                <span class="mx-2">•</span>
                                <span>Qty: <span class="font-semibold">{{ $batch['remaining_qty'] }}</span></span>
                                <span class="mx-2">•</span>
                                <span>Cost: {{ number_format((float)$batch['cost_price'], 2) }}</span>
                            </div>
                        </div>
                        @if (auth()->user()->can('clearance.send'))
                        <button type="button" wire:click="sendBatchToClearance({{ $batch['id'] }})"
                            class="ml-3 px-3 py-1 text-sm font-medium bg-amber-50 text-amber-700 hover:bg-amber-100 rounded-md transition-colors">
                            Send
                        </button>
                        @endif
                    </div>
                ...
                <div class="text-center py-8">
                    <p class="text-slate-600">No expired batches found for this product.</p>
                </div>
                @endif
            </div>

            <div class="flex items-center justify-end gap-2 border-t border-slate-200 p-4">
                <button type="button" wire:click="closeBatchDetailsModal()" class="ui-btn-secondary">Close</button>
            </div>
        </div>
    </div>
    @endif
